...
public sealed class EventApiService : IEventApiService
{
    ...
    public async Task<(bool Ok, Guid? AssignmentId, string? Error)> AssignCrewAsync(Guid eventId, Guid? crewId, Guid? vendorId, Guid? shiftId = null, CancellationToken ct = default)
    {
        try
        {
            var resp = await _http.PostAsJsonAsync($"api/v1/events/{eventId}/assignments", new { crewId, vendorId, shiftId }, ct);
    ...
    public async Task<(bool Ok, string? Error)> VendorRevokeCrewInviteAsync(
        Guid eventId, Guid crewId, CancellationToken ct = default)
    {
        try
        {
            var resp = await _http.DeleteAsync(
                $"api/v1/events/{eventId}/vendor-crew-invitations/{crewId}", ct);
            if (resp.IsSuccessStatusCode) return (true, null);
            var err = await resp.Content.ReadFromJsonAsync<ApiResult<object>>(_jsonOpts, ct);
